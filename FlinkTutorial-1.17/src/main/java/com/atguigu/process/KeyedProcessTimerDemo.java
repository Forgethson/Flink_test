        ...
        SingleOutputStreamOperator<String> process = sensorKS.process(
                new KeyedProcessFunction<String, WaterSensor, String>() {
                    ...
                    public void processElement(WaterSensor value, Context ctx, Collector<String> out) {
                        ...
                        // 1、事件时间的案例
                        Long currentEventTime = ctx.timestamp(); // 数据中提取出来的事件时间（等同value.getTs()）
                        timerService.registerEventTimeTimer(currentEventTime + 5000L);
                        System.out.println("当前key=" + currentKey + ",事件时间=" +
                                currentEventTime + ",注册了一个5s的定时器" + ",处理时间：" + System.currentTimeMillis() + ",wm：" + timerService.currentWatermark());

                        // 2、处理时间的案例
//                        long currentTs = timerService.currentProcessingTime();
//                        timerService.registerProcessingTimeTimer(currentTs + 5000L);
//                        System.out.println("当前key=" + currentKey + ",处理时间=" + currentTs + ",注册了一个5s后的定时器" + ",处理时间：" + System.currentTimeMillis());
...
//                        timerService.deleteProcessingTimeTimer();
                        ...
                        // 获取当前时间进展： 处理时间=当前系统时间，事件时间 >= watermark
//                        long currentTs = timerService.currentProcessingTime();
//                        long wm = timerService.currentWatermark();
                     ...
                     * @param timestamp 当前时间进展，就是定时器触发时的时间
                     * @param ctx       上下文
                     * @param out       采集器
                     * @throws Exception
                     */
                    ...
                    public void onTimer(long timestamp, OnTimerContext ctx, Collector<String> out) throws Exception {
                        ...
                        System.out.println("key=" + currentKey + ", 现在时间是" + timestamp +
                                "定时器触发" + "，处理时间：" + System.currentTimeMillis()+ ",wm：" + ctx.timerService().currentWatermark());
                    }
                }
        );
        process.print();
